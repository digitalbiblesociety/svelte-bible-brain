<script>
	...
	onMount(async () => {
		bibles = await fetch(`https://4.dbt.io/api/bibles?v=4&key=${key}&media=audio`).then(function (response) {
	...
	async function fetchAudioFileSet(bibleId,filesets) {
		...
		let books = await fetch(`https://4.dbt.io/api/bibles/${bibleId}/book?v=4&key=${key}`).then(function (response) {
			return response.json()
		})
		current_books = books['data']
	...
	async function setCurrentChapter(bible,book,chapter) {
		...
		current_chapter = await fetch(`https://4.dbt.io/api/bibles/filesets/${bible}/${book}/${chapter}?v=4&key=${key}`).then(function (response) {
			return response.json()
		})
		current_chapter = current_chapter['data'][0].path
		console.log(current_chapter)
		console.log(current_chapter.path)
	}
  ...
  <div class="container mx-auto py-12 px-8 relative" style="height:calc(100vh - 8rem)">
	<div class="flex w-full rounded-lg h-full lg:overflow-hidden overflow-auto flex-row shadow-2xl">
	  <div class="lg:w-1/2 text-gray-800 flex flex-col">
		<div class="p-8 shadow-md relative bg-gray-600 text-white">
		  <div class="flex items-center">
			...
			<svg class="h-10 block mx-auto object-cover object-top" fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 501 120">
		...
		<div class="overflow-y-scroll flex-grow">
			{#if bibles && current_books.length == 0}
	  ...
	  <div class="lg:w-1/2 bg-gray-600 text-white flex flex-col">
		<div class="p-8 bg-gray-700 grid grid-cols-3 divide-x-2 gap-2">
		...
		<AudioPlayer urls={[current_chapter]} showPrev={false} showNext={false} showShuffle={false} />
